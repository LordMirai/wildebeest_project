...
namespace Chess
{
    public class Movement
    {
        ...
        PieceMove?[] Moves()
        {
            PieceMove?[] moves = new PieceMove?[16];
            move = 0;
            switch (pieceTile.piece.piecekind)
            {
                // Bishop's move coordinates
                case PieceKind.Bishop: 
                    moves = new PieceMove?[32]; 
                    LineMove(moves, false);
                    break;
                // Knight's move coordinates
                case PieceKind.Knight:
                    ...
                    break;
                // Wildebeest's move coordinates
                case PieceKind.Wildebeest:
                    moves[0] = destination(2, -1);
                    moves[1] = destination(2, 1);
                    moves[2] = destination(-2, 1);
                    moves[3] = destination(-2, -1);
                    ...
                    moves[6] = destination(-1, -2);
                    moves[7] = destination(-1, 2); 
                    moves[8] = destination(3, -1);
                    moves[9] = destination(3, 1);
                    moves[10] = destination(-3, 1);
                    moves[11] = destination(-3, -1);
                    moves[12] = destination(1, -3);
                    moves[13] = destination(1, 3);
                    moves[14] = destination(-1, -3);
                    moves[15] = destination(-1, 3);
                    break;
                // Camel's move coordinates
                case PieceKind.Camel:
                    moves[0] = destination(3, -1);
                    moves[1] = destination(3, 1);
                    moves[2] = destination(-3, 1);
                    moves[3] = destination(-3, -1);
                    moves[4] = destination(1, -3);
                    moves[5] = destination(1, 3);
                    moves[6] = destination(-1, -3);
                    moves[7] = destination(-1, 3);
                    break;
                // Queen's move coordinates
                case PieceKind.Queen:
                    ...
                    LineMove(moves, true);
                    break;
                // Rook's move coordinates
                case PieceKind.Rook: 
                    moves = new PieceMove?[34]; 
                    LineMove(moves, true); 
                    break;
                // King's move coordinates
                case PieceKind.King:
        ...
        { // for pieces like Bishop, Queen and Rook 
            int rookDir = straightLine ? 0 : 1; int bishopDir = straightLine ? 1 : -1;
            for (int i = 1; i < 11; i++) moves[move] = LineDestination(-i, i * bishopDir * rookDir);
            previousMove = new PieceMove(1, 1); for (int i = 1; i < 11; i++) moves[move] = LineDestination(i, i * rookDir);
            previousMove = new PieceMove(1, 1); for (int i = 1; i < 11; i++) moves[move] = LineDestination(i * rookDir, -i);
            previousMove = new PieceMove(1, 1); for (int i = 1; i < 11; i++) moves[move] = LineDestination(i * bishopDir * rookDir, i);
        }
        ...
        { // for pieces like Knight and King
            ...
            if (isAccessAble(yLocation, xLocation))
            {
                ...
                if (isOccupied(yLocation, xLocation))
        ...
        {
            if (isAccessAble(y, x)) 
                return isOccupied(y, x) && Board.GetPiece(y, x).TwoTilesMove && isEnemyColor(y, x);
            else return false;
        }
