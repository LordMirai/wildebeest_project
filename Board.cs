...
namespace Chess
{
    class Board
    {
        public static Tile[,] tiles = new Tile[10, 11];
        ...
        public Board(Chess chess)
        {
            Window = chess;
            byte rows = 0;
            for (byte x = 0; x < 11; x++, rows++)
            {
                for (byte y = 0; y < 10; y++, rows++)
                {
        ...
        public static bool isAccessAble(int y, int x) => y < 10 && y >= 0 && x < 11 && x >= 0;
        private void SetPieces(int y, ChessColor color)
        {
            ...
            tiles[y, 3].piece = new ChessPiece(PieceKind.Bishop, color);
            tiles[y, 4].piece = new ChessPiece(PieceKind.Queen, color);
            tiles[y, 5].piece = new ChessPiece(PieceKind.King, color);
            tiles[y, 6].piece = new ChessPiece(PieceKind.Wildebeest, color);
            tiles[y, 7].piece = new ChessPiece(PieceKind.Camel, color);
            tiles[y, 8].piece = new ChessPiece(PieceKind.Camel, color);
            tiles[y, 9].piece = new ChessPiece(PieceKind.Knight, color);
            tiles[y, 10].piece = new ChessPiece(PieceKind.Rook, color);
            y += y == 0 ? 1 : -1;
            for (int x = 0; x < 11; x++)
